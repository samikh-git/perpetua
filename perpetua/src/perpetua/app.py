...
@app.command()
def config():
    """Creates a config folder in the home root of user called perpetua. This is a required command to use the package."""
    from rich.prompt import Prompt
    from rich.live import Live

    if os.path.exists(HOME_DIR + "/perpetua"):
        console.print(f"[yellow]The config directory has already been created. Please check {HOME_DIR + '/perpetua'}")
    else:
        ...
        os.mkdir(HOME_DIR + "/perpetua")
        with open(HOME_DIR + "/perpetua/.env", "x") as f:
            f.write(env_file)
        console.print(f"""[green]Created config file in {HOME_DIR + "/perpetua"}.""")

@app.command()
def init():
    """Initializes Perpetua project by creating .rag directory.
    
    This is required to use Perpetua in a project.
    """
    ...
    try:
        with console.status("intializing..."):
            ...
            with open(HOME_DIR + f"/perpetua/{thread}.txt", "x") as f:
                f.write(f"Conversations for thread id {thread} and Perpetua project {os.getcwd()} \n")

            create_repo_structure_doc()
        console.print("[green]Created .rag directory. Your perpetua project has been initialized!")
    except FileExistsError as e:
       console.print("[red]This is already a Perpetua project!")
...
def search(query: str):
    ...
    try:
        assert check_initialization(), "This is not a perpetua project! Please initialize this repo."
...
def ask(save: bool = False):
    ...
    if save:
        with open(HOME_DIR + f"/perpetua/{thread}.txt", "a") as f:
...
def status():
    ...
    if check_initialization():
        ...
        console.print("[red] This is not a Perpetua project. Please initialize.")
